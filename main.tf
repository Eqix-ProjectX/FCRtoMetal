...
# to create BGP inside FCR for its connection facing Metal

resource "equinix_fabric_routing_protocol" "L3_FCRDA_to_MetalDA_FCRside123" {
  depends_on = [equinix_fabric_connection.primary_cloud_router_connection123]
  connection_uuid = equinix_fabric_connection.primary_cloud_router_connection123.id
  type            = "DIRECT"
  name = var.FCR_Layer3connection_name
  direct_ipv4 {
    equinix_iface_ip = var.BGPIP_on_FCR

  }
}

resource "equinix_fabric_routing_protocol" "_L3_FCRDA_to_MetalDA_Metalside123" {
  depends_on = [
    equinix_fabric_routing_protocol.L3_FCRDA_to_MetalDA_FCRside123
  ]
connection_uuid = equinix_fabric_connection.primary_cloud_router_connection123.id
  type            = "BGP"
  customer_asn    = var.Metal_ASN
  name = var.Metal_Layer3connection_name
  bgp_ipv4 {
    customer_peer_ip = var.MetalIP_on_FCR
    enabled          = true
  }
